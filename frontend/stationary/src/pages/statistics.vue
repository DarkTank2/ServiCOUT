        ...
        <v-row>
            <v-col cols="12">
                <v-window v-model="windowModel" show-arrows>
                    ...
                    <v-window-item :style="{ height: '600px'}">
...
import { Chart as ChartJS, Title, Tooltip, Legend, LineElement, CategoryScale, LinearScale, type ChartData, PointElement } from 'chart.js'
...
const { isLoading } = useConfigLoader()
ChartJS.register(
    CategoryScale,
    LinearScale,
    PointElement,
...
const { data: allOrderedItems } = toRefs(api.service('ordered-items').findInStore(computed(() => ({ query: { notCashed: 0 }, $limit: countedOrderedItems.total }))))

const countedOrders = api.service('orders').countInStore(computed(() => ({ query: { id: { $in: allOrderedItems.value.map(({ orderId }) => orderId!) } } })))
const { data: allOrders } = toRefs(api.service('orders').findInStore(computed(() => ({ query: { id: { $in: allOrderedItems.value.map(({ orderId }) => orderId!) } }, $limit: countedOrders.total }))))
...
const { data: allOrdersByName } = toRefs(api.service('orders').findInStore(computed(() => ({ query: { waiter: selectedUsername.value }, $limit: countedOrdersByName.total }))))
...
const { data: allOrderedItemsByName } = toRefs(api.service('ordered-items').findInStore(computed(() => ({ query: { orderId: { $in: allOrdersByName.value.map(({ id }) => id!) }, notCashed: 0 }, $limit: countedOrderedItemsByName.total }))))
...
const { data: allOrderedItemsByNameAndSelection } = toRefs(api.service('ordered-items').findInStore(computed(() => ({ query: { orderId: { $in: allOrdersByName.value.map(({ id }) => id!) }, itemId: { $in: selectedItems.value }, notCashed: 0, $sort: { id: 1 } }, $limit: countedOrderedItemsByNameAndSelection.total }))))
...
const { data: allSizes } = toRefs(api.service('sizes').findInStore(computed(() => ({ query: { $limit: countedSizes.total } }))))
const { data: allFlavours } = toRefs(api.service('flavours').findInStore(computed(() => ({ query: { $limit: countedFlavours.total } }))))
const { data: allBaseItems } = toRefs(api.service('base-items').findInStore(computed(() => ({ query: { $limit: countedBaseItems.total } }))))
const { data: allCategories } = toRefs(api.service('categories').findInStore(computed(() => ({ query: { $limit: countedCategories.total } }))))
...
const updateSelection = async function (changedProperty: 'username' | 'categories' | 'base-items' | 'items') {
    if (changedProperty === 'username' || changedProperty === 'items') {
        ...
        selectedBaseItems.value = selectedCategories.value.flatMap(id => {
            return allBaseItems.value.filter(({ categoryId }) => categoryId === id).map(({ id }) => id!)
        })
        selectedItems.value = selectedBaseItems.value.flatMap(id => {
...
<script lang="ts">
import { useConfigLoader } from '@/loaders/config-loader'
