            ...
            <template #top>
                <v-toolbar flat>
                    <new-item-dialog v-if="!denyCreation" :headers="headers" :service-name="serviceName">
                        ...
                        <template #dialog-title>Erstelle ein neues Produkt</template>
                    </new-item-dialog>
...
import type { ServiceTypes, ItemsData } from 'backend';
type DataType = ItemsData
const serviceName: keyof ServiceTypes = 'items'
...
const headers = computed(() => {
    let headers: Array<I> = [{
        ...
        title: 'Basis-Produkt',
        type: 'select',
        selectFrom: api.service('base-items').store.items as Array<any>
    }, {
        value: 'sizeId',
        readonly: true,
        title: 'Größe',
        type: 'select',
        selectFrom: api.service('sizes').store.items as Array<any>
    }, {
        value: 'flavourId',
        readonly: true,
        title: 'Ausrichtung',
        type: 'select',
        selectFrom: api.service('flavours').store.items as Array<any>
    }, {
        value: 'price',
        readonly: true,
        title: 'Preis',
