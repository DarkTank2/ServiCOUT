    ...
    <v-card>
        ...
        <v-card-text :style="{ padding: 0 }">
            <v-list>
                ...
                <v-divider></v-divider>
                <v-list-item>
                    <v-text-field :model-value="settings.getName" @update:model-value="settings.setName" label="Dein Name"
...
const finishOrder = async function () {
    ...
    let order = await api.service('orders').create({ waiter: settings.getName!, tableId: mobileSettings.getTableId!, tenantId: api.service('tenants').findInStore({}).data[0]!.id! })
