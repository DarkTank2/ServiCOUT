...
export const useCalculatorStore = defineStore('calculator', () => {
    ...
    const updateSeparateMode = function (val: boolean | null) {
        if (val === null) {
            separatedMode.value = false
        } else {
            separatedMode.value = val
        }
        window.localStorage.setItem('separatedMode', val ? 'true' : 'false')
    }
    const updateQuickMode = function (val: boolean | null) {
        if (val === null) {
    ...
    const updateEditMode = function (val: boolean | null) {
        if (val === null) {
    ...
    const updateImmediatelyFinishedMode = function (val: boolean | null) {
        if (val === null) {
            immediatelyFinishedMode.value = false
        } else {
