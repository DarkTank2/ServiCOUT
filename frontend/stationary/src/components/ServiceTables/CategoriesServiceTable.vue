<template>
    <v-card class="pa-2 my-2">
        <v-card-title class="d-flex align-center pe-2">
            <span>{{ texts.serviceTableTitle }}</span>
        </v-card-title>
        ...
        <v-data-table :items="clonedItems" :search="search"
            ...
            <template #top>
                <v-toolbar flat>
                    ...
                    :default-config="{ available: true }">
                        <template #button>
                            <v-icon icon="mdi-plus"></v-icon>
                            <span>{{ texts.createButtonText }}</span>
                        </template>
                        <template #dialog-title>{{ texts.createDialogTitle }}</template>
                    </new-item-dialog>
...
import type { ServiceTypes, CategoriesData } from 'backend';
type DataType = CategoriesData
const serviceName: keyof ServiceTypes = 'categories'

const texts = ref({
    serviceTableTitle: 'Kategorien',
    createButtonText: 'Neue Kategorie anlegen',
    createDialogTitle: 'Erstelle einer neuen Kategorie',
    nameText: 'Name der Kategorie'
})
