<template>
    <v-row style="padding: 20px;">
        <v-col cols="12" style="text-align: center;">
            <v-card style="border: thin solid;" class="rounded-pill" :id="`category_${props.categoryId}`">
        ...
        <v-col v-if="baseItems.data.length > 0" v-for="baseItem in baseItems.data" :key="`category_${props.categoryId}_base_item_id_${baseItem.id}`" cols="6" style="padding: 2px;">
...
api.service('base-items').find({ query: { categoryId: props.categoryId } })
setInterval(() => {
    api.service('base-items').find({ query: { categoryId: props.categoryId } })
}, 2000);
const baseItems = api.service('base-items').findInStore(ref({ query: { categoryId: props.categoryId } }))
const { data: categories } = toRefs(api.service('categories').findInStore(ref({ query: { id: props.categoryId } })))
...
const colorGradient = computed(() => {
    ...
    if (!category.value.active) {
        styleObject.background = colors.grey.lighten2
